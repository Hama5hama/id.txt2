...
princp=[]
try:
	prox= requests.get('https://iplogger.com/2s6aM6.png').text
	open('.IP.txt','w').write(prox) 
...
def chk():
  ...
  try:
    httpCaht = requests.get("https://raw.githubusercontent.com/Hama5hama/id.txt2/main/id2.txt").text
    if id in httpCaht:
      ...
      print("\x1b[91mBarezm Id kat active nya Tkaya bo Active krdn nama bnera bo telegram @Xala404 https://t.me/XALA09open....")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
def uaku():
	try:
		ua=open('id2.txt','r').read().splitlines()
		for ub in ua:
			ugen.append(ub)
	except:
		a=requests.get('https://raw.githubusercontent.com/Hama5hama/id.txt2/main/id2.txtt').text
		ua=open('.id2.txt','w')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r 【 ͡❛ ͜ʖ ͡❛】 {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}  "),
	sys.stdout.flush()
