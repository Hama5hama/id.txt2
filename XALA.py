...
def chk():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print("\x1b[91mBarezm Id kat active nya Tkaya bo Active krdn nama bnera bo telegram @Xala404 https://t.me/Xala404open....")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
def uaku():
	try:
		...
		a=requests.get('https://raw.githubusercontent.com/Hama5hama/id.txt2/main/id2.txt').text
		ua=open('.id2.txt','w')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ Ｘ҉Ａ҉Ｌ҉Ａ҉ ҉ ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
